      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <div className="max-h-80 overflow-y-auto">
              {loading ? (
                ...
                <div className="py-10 text-center">
                  <BellIcon className="w-10 h-10 mx-auto mb-2" style={{ color: 'var(--text-faint)' }} />
                  <p className="text-sm" style={{ color: 'var(--text-faint)' }}>{t('notifications.empty')}</p>
                </div>
              ) : (
                notifications.map((n, i) => (
                  <div
                    ...
                    style={{
                      borderColor: 'var(--border)',
                      background: !n.read ? 'rgba(59,130,246,0.06)' : 'transparent',
                    }}
                    onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-hover)'}
                    onMouseLeave={e => e.currentTarget.style.background = !n.read ? 'rgba(59,130,246,0.06)' : 'transparent'}
                  >
                    <span className="text-xl mt-0.5" onClick={() => { if (!n.read) markAsRead(n.id); setOpen(false) }}>{typeIcon[n.action] || '🔔'}</span>
                    <div className="flex-1 min-w-0" onClick={() => { if (!n.read) markAsRead(n.id); setOpen(false) }}>
                      <p className="text-sm leading-snug" style={{ color: 'var(--text)' }}>{n.description}</p>
            ...
            {/* Footer */}
            <div className="px-5 py-3 border-t" style={{ borderColor: 'var(--border)', background: 'var(--bg-muted)' }}>
              <button
