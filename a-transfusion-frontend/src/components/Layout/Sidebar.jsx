...
const buildMenu = (t) => ({
  admin: [
    ...
    { path: '/demandes',     label: t('nav.requests'),    icon: ClipboardDocumentListIcon },
        ...
        <AnimatePresence mode="wait">
          {!collapsed ? (
            <motion.div key="exp" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity" onClick={onLogoClick}>
              <Logo />
              <span className="text-xs font-bold whitespace-nowrap" style={{ color: 'var(--text)' }}>HemoConnect CHU</span>
            </motion.div>
          ) : (
            <motion.div key="col" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="flex justify-center cursor-pointer hover:opacity-80 transition-opacity" onClick={onLogoClick}><Logo /></motion.div>
          )}
        </AnimatePresence>
      </div>
          ...
          >
            <div className="flex items-center justify-between px-4 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
              <div className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity" onClick={onLogoClick}>
                <Logo />
                <span className="text-sm font-bold" style={{ color: 'var(--text)' }}>HemoConnect CHU</span>
              </div>
            ...
            <nav className="flex-1 px-2 py-3 space-y-0.5">
              {items.map(item => (
                <SidebarLink key={item.path} item={item} collapsed={false} onNavigate={onClose} />
              ))}
            </nav>
          </motion.div>
        </>
  ...
  >
    {({ isActive }) => (
      <>
        ...
        {isActive && (
          <motion.div layoutId="sidebarLine"
...
const Logo = () => (
  <img src="/logo.png" alt="HemoConnect CHU" className="w-7 h-7 object-contain drop-shadow-sm rounded-md" />
)

// Default export for backwards compat (desktop only)
const Sidebar = ({ collapsed, onToggle }) => <DesktopSidebar collapsed={collapsed} onToggle={onToggle} />
