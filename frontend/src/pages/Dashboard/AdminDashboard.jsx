...
import {
  UserGroupIcon, ClipboardDocumentListIcon, CubeIcon,
...
const StatCard = ({ title, value, icon: Icon, gradient, trend }) => (
  ...
  >
    {/* Glow blob */}
    <div className={`absolute -top-6 -left-6 w-24 h-24 rounded-full opacity-10 ${gradient}`} />
...
const AdminDashboard = () => {
  ...
  const roleLabel = (role) => ({
    admin: t('auth.role_admin'), hospital: t('auth.role_hospital'), blood_center: t('auth.role_blood_center'),
  })[role] || role

  /* Status labels translated */
  const statusLabel = (s) => ({
    pending: t('status.pending'), approved: t('status.approved'),
    rejected: t('status.rejected'), completed: t('status.completed'),
  })[s] || s
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <Button variant="outline" size="sm" onClick={() => fetchStats(true)} loading={refreshing} icon={ArrowPathIcon}>
          {t('common.refresh')}
        </Button>
      </motion.div>
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <Button onClick={() => fetchStats()} variant="outline" size="sm">{t('common.refresh')}</Button>
        </motion.div>
      ) : (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible"
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          <StatCard title={t('users.title').replace('Gestion des ', '').replace(' Management','')} value={stats?.users_count || 0}              icon={UserGroupIcon}             gradient="bg-blue-500" />
          <StatCard title={t('stats.total_requests')} value={stats?.demandes_count || 0}           icon={ClipboardDocumentListIcon} gradient="bg-emerald-500" />
          <StatCard title={t('stats.pending')}         value={stats?.pending_demandes || 0}         icon={ClockIcon}                 gradient="bg-amber-400" />
          <StatCard title={t('stats.total_blood_units')} value={stats?.blood_stock_summary?.total_units || 0} icon={CubeIcon}          gradient="bg-red-500" />
        </motion.div>
      )}
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible"
                ...
                <PieChart>
                  <Pie
                    data={Object.entries(stats?.users_by_role || {}).map(([role, count]) => ({
                      name: roleLabel(role), value: count,
                    }))}
                    cx="50%" cy="50%" outerRadius={90} innerRadius={45} dataKey="value"
          ...
          <motion.div variants={staggerItem}>
            <Card title={t('stats.by_status')}>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart
                  data={Object.entries(stats?.demandes_by_status || {}).map(([s, count]) => ({
                    status: statusLabel(s), count,
                  }))}
                  layout="vertical" margin={{ top: 5, right: 20, left: 60, bottom: 5 }}
                >
                  <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="var(--border)" />
                  <XAxis type="number" tick={{ fontSize: 11, fill: 'var(--text-muted)' }} />
                  <YAxis type="category" dataKey="status" tick={{ fontSize: 11, fill: 'var(--text-muted)' }} />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Bar dataKey="count" fill="#3b82f6" radius={[0, 6, 6, 0]}
                    isAnimationActive animationBegin={400} animationDuration={600} />
                </BarChart>
              </ResponsiveContainer>
            </Card>
          </motion.div>
      ...
      {!loading && !error && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.4 }}>
          <Card title={t('stats.monthly_transfusions')}>
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={stats?.transfusions_by_month || []} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
      ...
      {/* Blood stock table */}
      {!loading && !error && stats?.blood_stock_summary?.by_type?.length > 0 && (
          ...
          <Card title={t('stock.title')} noPad>
            <div className="t-wrap">
              <table className="t-table">
                ...
                <motion.tbody variants={staggerContainer} initial="hidden" animate="visible" className="t-body">
                  {stats.blood_stock_summary.by_type.map((stock, idx) => (
                    <motion.tr key={idx} variants={staggerItem}>
                      <td className="t-td font-bold text-red-500">{stock.blood_type}</td>
                      <td className="t-td">
                        <span className="font-semibold" style={{ color: 'var(--text)' }}>{stock.quantity}</span>
                        <span className="ml-1 text-xs" style={{ color: 'var(--text-faint)' }}>{t('stock.units')}</span>
                      </td>
            ...
            <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-1">
              {stats.recent_activities.slice(0, 10).map((a, idx) => (
                <motion.div key={idx} variants={staggerItem}
                  ...
                  <span className="flex-shrink-0 text-xs px-2.5 py-1 rounded-lg font-medium"
