...
import {
  CubeIcon, ClipboardDocumentListIcon, ClockIcon, CheckCircleIcon,
...
const BloodCenterDashboard = () => {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const [sR, stR] = await Promise.all([dashboardService.getStats(), stockService.getStock()])
      setStats(sR.data); setStock(stR.data || [])
    ...
    <div className="space-y-5">
      ...
      {lowStock.length > 0 && (
        <div className="flex items-start gap-3 p-4 rounded-xl border"
          ...
          <p className="text-sm font-medium">
            {lowStock.length} {lowStock.length === 1 ? 'groupe sanguin' : 'groupes sanguins'} {t('stock.low').toLowerCase()}
          </p>
        </div>
      )}

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title={t('stats.total_blood_units')} value={stats?.total_blood_units}         icon={CubeIcon}                  colorClass="bg-red-500" />
        <StatCard title={t('stats.pending')}           value={stats?.pending_demandes}          icon={ClockIcon}                 colorClass="bg-amber-500" />
        <StatCard title={t('stats.todays_requests')}   value={stats?.today_transfusions}        icon={CheckCircleIcon}           colorClass="bg-emerald-500" />
        <StatCard title={t('stats.urgent_pending')}    value={stats?.urgent_pending_demandes}   icon={ExclamationTriangleIcon}   colorClass="bg-purple-500" />
      </div>
      ...
      <Card title={t('stock.title')}>
            ...
            <tbody className="t-body">
              {stock.map(item => (
                <tr key={item.id}>
                  ...
                  <td className="t-td">
                    <span className={`font-bold ${item.quantity === 0 ? 'text-red-500' : item.is_low ? 'text-amber-500' : 'text-emerald-500'}`}>
                      {item.quantity}
                    </span>
                  </td>
                  <td className="t-td t-td-muted">{item.minimum_threshold}</td>
                  <td className="t-td">{stockBadge(item)}</td>
                  <td className="t-td t-td-muted">{item.last_updated ? new Date(item.last_updated).toLocaleDateString() : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="mt-4 text-center">
          <Link to="/stock"><Button variant="outline" size="sm">{t('stock.title')}</Button></Link>
        </div>
      </Card>
      ...
      {stats?.recent_demandes && (
        <Card title={t('demandes.recent')}>
            ...
            <table className="t-table">
              ...
              <tbody className="t-body">
                {stats.recent_demandes.map(d => (
                  <tr key={d.id}>
                    <td className="t-td t-td-muted">{d.hospital?.name || '—'}</td>
                    ...
                    <td className="t-td">
                      <span className={{ emergency: 'badge badge-red', urgent: 'badge badge-yellow', normal: 'badge badge-green' }[d.urgency] || 'badge badge-gray'}>
          ...
          <div className="mt-4 text-center">
            <Link to="/demandes"><Button variant="outline" size="sm">{t('demandes.view_all')}</Button></Link>
          </div>
        </Card>
      )}
    </div>
