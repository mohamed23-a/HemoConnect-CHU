    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      >
        <h1 className="page-title">
          {isEditMode
            ? t("create.edit_title") || "Edit Request"
            : t("create.title")}
        </h1>
      ...
      <Card>
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            <input
              className={inputCls("patient_name")}
              type="text"
              name="patient_name"
              value={form.patient_name}
              onChange={change}
              placeholder={t("create.patient_name_ph")}
            />
          </Field>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            >
              <input
                className={inputCls("patient_age")}
                type="number"
                name="patient_age"
                value={form.patient_age}
                onChange={change}
                placeholder={t("create.patient_age_ph")}
                min="0"
                max="120"
              />
            </Field>
            ...
            <Field
              label={t("create.product_type") || "Product Type"}
              error={errors.product_type}
              required
            >
              <select
                className={inputCls("product_type")}
                name="product_type"
                value={form.product_type}
                onChange={change}
              >
                <option value="">
                  {t("create.product_type_ph") || "Select Product"}
                </option>
          ...
          <Field label={t("create.reason")} error={errors.reason} required>
            <textarea
              ...
              value={form.reason}
              onChange={change}
              placeholder={t("create.reason_ph")}
            />
          </Field>
          ...
          >
            ...
            <Button type="submit" variant="primary" loading={loading}>
              {isEditMode
                ? t("create.save_changes") || "Save Changes"
                : t("create.submit")}
            </Button>
          </div>
        </form>
