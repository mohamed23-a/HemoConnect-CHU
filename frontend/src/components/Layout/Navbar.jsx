...
const Navbar = ({ onMobileMenuToggle, onLogoClick }) => {
  ...
  const roleStyle = roleColors[user?.role] || { bg: 'var(--bg-muted)', color: 'var(--text-muted)' }
      ...
      <div className="px-4 sm:px-6">
        <div className="flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity" onClick={onLogoClick} title="À propos / About HemoConnect CHU">
              <img src="/logo.png" alt="HemoConnect" className="w-7 h-7 object-contain drop-shadow-sm rounded-md" />
              <span className="text-sm font-bold gradient-text hidden sm:block">HemoConnect CHU</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1">
                  ...
                  <DropdownPanel style={{ [isRTL ? 'right' : 'left']: 0 }}>
                    {LANGS.map(({ code, label, flag, dir }) => (
                      <button
                        ...
                        style={{
                          color: i18n.language === code ? '#3b82f6' : 'var(--text)',
            ...
            <motion.button
              ...
              onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
            >
              <motion.div animate={{ rotate: isDark ? 180 : 0 }} transition={{ duration: 0.35 }}>
                {isDark
            ...
            <div className="relative" ref={userRef}>
              ...
              >
                ...
                <ChevronDownIcon className="w-3.5 h-3.5 hidden sm:block" style={{ color: 'var(--text-faint)' }} />
              </button>

              <AnimatePresence>
                {userOpen && (
                  <DropdownPanel style={{ [isRTL ? 'left' : 'right']: 0, minWidth: '210px' }}>
                    {/* Header */}
                    ...
                    <div className="py-1">
                      <button
                        ...
                        onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
                      >
                        <KeyIcon className="w-4 h-4" style={{ color: 'var(--text-faint)' }} />
